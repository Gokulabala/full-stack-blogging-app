pipeline {
    ...
    stages {
        stage('Git Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/Gokulabala/full-stack-blogging-app.git'
            }
        }
        stage('Compile') {
            steps {
        ...
        stage('Docker Build & Tag') {
            steps {
                ...
                sh "docker build -t gokulabalan/blog-app ."
                }
                }
            }
        }
        ...
        stage('Docker Push Image') {
            steps {
                script{
                withDockerRegistry(credentialsId: 'dockerhub-cred', url: 'https://index.docker.io/v1/') {
                    sh "docker push gokulabalan/blog-app"
                }
                }
            }
        }
        stage('K8s Deploy') {
            steps {
               withKubeCredentials(kubectlCredentials: [[caCertificate: '', clusterName: ' devopsshack-cluster', contextName: '', credentialsId: 'k8s-token', namespace: 'webapps', serverUrl: 'https://178CF4D541A2DD276A5DB3D87DA85B14.gr7.eu-north-1.eks.amazonaws.com']]) {
